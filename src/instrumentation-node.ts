...
export async function registerNodejs(): Promise<void> {
  ...
  // Pre-initialize database if using asynchronous WASM SQLite (Termux/Android)
  try {
    const { ensureDbInitialized } = await import("@/lib/db/core");
    await ensureDbInitialized();
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[STARTUP] Fatal: Database pre-initialization failed:", msg);
    throw err;
  }
  ...
  // Database already pre-initialized at startup
